...
2. Run the following commands:
   ...
   docker run --rm -d -v $(pwd)/output:/output myprogram
   ```
...
2. Use the following commands:
   ...
   - **Command Prompt**:
     ```cmd
     docker build -t myprogram .
     docker run --rm -d -v %cd%/output:/output myprogram
     ```
...
- `docker run --rm -d -v $(pwd)/output:/output myprogram`: Runs the `myprogram` container, mounts the local `output` directory into the container’s `/output` directory, and automatically removes the container after it exits.
